...
const forwardJsonRequest = async (
  ...
  if (proxyServer) {
    const curlResponse = await forwardJsonRequestWithCurl(targetUrl, apiKey, payload, proxyServer);
...
const forwardJsonRequestWithRetry = async (
  ...
  let upstream = await forwardJsonRequest(targetUrl, apiKey, payload, proxyServer);
...
export default defineConfig(({ mode }) => {
  ...
  const comflyApiKey = env.VITE_COMFLY_API_KEY || env.COMFLY_API_KEY || env.VITE_DEFAULT_IMAGE_API_KEY || env.DEFAULT_IMAGE_API_KEY || '';
        ...
        configureServer(server) {
          ...
          server.middlewares.use('/api/openai-chat-proxy', async (req, res) => {
            ...
            try {
              ...
              const openaiProxyServer = env.VITE_OPENAI_PROXY_SERVER?.trim() || getWindowsHttpProxy();
              const upstream = await forwardJsonRequestWithRetry(targetUrl, apiKey, payload, openaiProxyServer);
