...
const prepareRequest = (config: SoraConfig) => {
    ...
    if (!useBaseUrl.endsWith('/v1') && config.type !== 'univ2' && config.type !== 'minimax') {
        useBaseUrl += '/v1';
    }
        ...
        if (config.type === 'minimax') {
            ...
            const imageInputs = images?.length ? images : (image ? [image] : []);
              ...
              for (let index = 0; index < imageInputs.length; index += 1) {
                  const role = imageInputs.length <= 2
                      ? (index === 0 ? 'first_frame' : 'last_frame')
                      : 'reference_image';
                  content.push({
                      type: 'image_url',
                      image_url: { url: await toBase64(imageInputs[index]) },
                      role
                  });
              }

            const duration = Math.min(15, Math.max(4, parseInt(seconds || '5', 10) || 5));
            const resolution = /2k/i.test(size) ? '2K' : '768P';
            const ratio = imageInputs.length > 0 ? 'adaptive' : (aspect_ratio || '16:9');
            response = await axios.post('/api/minimax-video-proxy', {
                action: 'create',
                baseUrl,
...
export const pollSoraTask = async (
    ...
    while (true) {
        ...
        try {
            ...
            if (config.type === 'minimax') {
                statusRes = await axios.post('/api/minimax-video-proxy', {
                    action: 'query',
                    baseUrl,
                    apiKey: config.apiKey,
                    taskId
                }, { signal });
                statusRes.data = statusRes.data.task || statusRes.data;
            } else if (config.type === 'univ2') {
