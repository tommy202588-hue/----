...
const ImageComposerModal: React.FC<ImageComposerModalProps> = ({ isOpen, onClose, initialImages = [], onSendToCard }) => {
    ...
    const getSelectedExportSize = () => {
        const [width, height] = exportSizeSelection.split(',').map(Number);
    ...
    const exportImage = async () => {
        ...
        const canvas = await generateResultCanvas(getSelectedExportSize());
        if (!canvas) return;
    ...
    const generateResultCanvas = async (targetSize?: { width: number; height: number } | number) => {
        ...
        const explicitSize = typeof targetSize === 'object' ? targetSize : undefined;
        const maxDim = typeof targetSize === 'number' ? targetSize : undefined;
        let scale = explicitSize ? explicitSize.width / EDIT_WIDTH : maxNeededScale;
        ...
        const targetW = explicitSize?.width || Math.round(EDIT_WIDTH * scale);
        const targetH = explicitSize?.height || Math.round(EDIT_HEIGHT * scale);
