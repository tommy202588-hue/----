...
const PromptPanel: React.FC<PromptPanelProps> = ({ initialPrompt = '', initialParams, status, nodeType, appSettings, providerId, modelId, upstreamImages = [], onGenerate, onPromptChange, onParamsChange, onProviderChange, onModelChange }) => {
    ...
    const [aspectRatio, setAspectRatio] = useState(initialParams?.aspectRatio || initialParams?.aspect_ratio || '16:9');
    const [resolution, setResolution] = useState(() => {
        // Initialize with correct default based on nodeType to avoid extra renders
        const initial = initialParams?.resolution || initialParams?.size;
        if (initial) return initial;
    ...
    const isMinimax = selectedProvider?.type === 'minimax' || model === 'MiniMax-H3';
    const displayResolutions = isAudio ? ['mp3'] : isGrok ? GROK_RESOLUTIONS : (isVideo ? (isMinimax ? MINIMAX_VIDEO_RESOLUTIONS : VIDEO_RESOLUTIONS) : IMG_RESOLUTIONS);
    ...
    }, [resolution, displayResolutions, selectedProvider?.id, model]);
    ...
    // App creates the upstream image array while rendering nodes. Track its
    // content instead of its identity so parent updates do not reset editor
    // DOM or steal the caret during typing.
    const upstreamImageSignature = upstreamImages
        .map(image => `${image.id}:${image.url}:${image.mention}`)
        .join('\u0001');
    ...
    }, [initialPrompt, characters, upstreamImageSignature]);
    ...
    const insertMentionAtCursor = (mentionValue: string, displayLabel: string, variant: 'character' | 'image', imageId?: string) => {
        ...
        const activeRangeInEditor = activeRange && contentRef.current?.contains(activeRange.commonAncestorContainer)
            ? activeRange
            : null;
        const range = savedRange || activeRangeInEditor || (() => {
        ...
        if (!savedRange && showMention && mentionQuery && range.startContainer.nodeType === Node.TEXT_NODE) {
